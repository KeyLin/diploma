...
if [ ! -d "${sphinxDir}/sphinxtrain" ]; then
    ...
    ./sphinxtrain-5prealpha/configure --prefix=${sphinxDir}/sphinxtrain && make && sudo make install || { echo "sphinxtrain install failed"; exit 1; }
...
if [ ! -d "${sphinxDir}/cmuclmtk" ]; then
    ...
    ./cmuclmtk-0.7/configure --prefix=${sphinxDir}/cmuclmtk && make && sudo make install|| { echo "cmuclmtk-0.7 install failed"; exit 1; }
